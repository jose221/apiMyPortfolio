...
class PortfoliosController {
    paramsCreate = Joi.object({
        ...
        knowledge_level: Joi.number(),
        portfolio_categories_id: Joi.number().required(),
    });
    ...
    paramsUpdate = Joi.object({
        ...
        years_experience: Joi.number(),
        knowledge_level: Joi.number(),
        portfolio_categories_id: Joi.number()
    });
    ...
    async update(req, res, token){
        ...
        try{
            if(req.params.id){
                if(req.files.icon_path) req.body.icon_path = await UploadFile.save(token,req.files.icon_path, {module:"portfolios", returnUrl:true, type:'image'} )
...
module.exports = new PortfoliosController();